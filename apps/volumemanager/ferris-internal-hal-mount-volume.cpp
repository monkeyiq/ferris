...
const string PROGRAM_NAME = "ferris-internal-hal-mount-volume";
...
//         }
    ...
    try
    {
        for( stringlist_t::iterator vi = volumes.begin(); vi != volumes.end(); ++vi )
        {
            string mp = "";
            string vol = *vi;
            cerr << "mounting volume:" << vol << endl;
            fh_context c = Resolve( vol );
            if( mp.empty() )
            {
                mp = getStrAttr( c, "volume.label", "" );
            }
            if( mp.empty() )
            {
                mp = getStrAttr( c, "storage.model", "" );
            }
            if( mp.empty() )
            {
                mp = c->getDirName();
            }
            setStrAttr( c, "mount", mp );
            LG_HAL_D << "mounted at:" << mp << " c:" << c->getURL() << endl;
            cerr << "mounted at:" << mp << " c:" << c->getURL() << endl;
        }
    }
    catch( exception& e )
    {
