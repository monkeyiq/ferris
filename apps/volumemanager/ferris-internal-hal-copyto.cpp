...
//         }
    ...
    try
    {
        string dsturl = dst_CSTR;
        
        for( stringlist_t::iterator vi = volumes.begin(); vi != volumes.end(); ++vi )
        {
            string vol = *vi;
            fh_context c = Resolve( vol );
            string mpurl = getStrAttr( c, "volume.mount_point", "" );
            fh_context mp  = Resolve(mpurl);
            fh_context dst = Resolve(dsturl);

            LG_HAL_D << "hal-copyto: Copy files from:" << mp->getURL()
                     << " to dst:" << dst->getURL()
                     << endl;
            fh_context clip = Factory::getFileClipboard("hal");
            FileClip::Copy( clip, mp );
            FileClip::Paste( clip, dst );
        }
    }
    catch( exception& e )
    {
