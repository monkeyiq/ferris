...
*******************************************************************************
    ...
    ferris-rdf-smush-union-urls.cpp
    ...
    $Id: ferris-rdf-smush-union-urls.cpp,v 1.4 2010/09/24 21:31:19 ben Exp $
...
const string PROGRAM_NAME = "ferris-rdf-smush-union-urls";
...
//                   "put redirection into this EA", "" },
        ...
        while( const char* tmpCSTR = poptGetArg(optCon) )
        {
            ...
            {
                cerr << "Warning, can't resolve src:" << srcURL << endl;
            }
            srcs.push_back( srcURL );
        }

        Semantic::myrdfSmushUnion( srcs );
    }
    catch( exception& e )
    {
