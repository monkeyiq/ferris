...
jpeg_data_append_section (JPEGData *data)
{
	...
	if (!data->count)
		...
		s = realloc (data->sections, sizeof (JPEGSection) * (data->count + 1));
	if (!s)
		return;

    s[ data->count ].marker = 0;
    s[ data->count ].content.generic.size = 0;
    s[ data->count ].content.generic.data = 0;
    data->sections = s;
	data->count++;
}
...
/*     printf("jpeg_data_save_data() data->count:%ld\n", data->count ); */
    
	for (*ds = i = 0; i < data->count; i++) {
		s = data->sections[i];
//#ifdef DEBUG
/* 		fprintf( stderr, "Writing marker 0x%x ('%s') at position %i...\n", */
/*                 s.marker, */
/*                 jpeg_marker_get_name (s.marker), */
/*                 *ds); */
//#endif
...
/*             fprintf(stderr, "jpeg_data_save_data() exif size:%ld\n", eds ); */
			*d = realloc (*d, sizeof (char) * (*ds + 2));
			(*d)[*ds + 0] = (eds + 2) >> 8;
			(*d)[*ds + 1] = (eds + 2) >> 0;
			*ds += 2;
...
//#ifdef DEBUG
/*         fprintf( stderr, "Found marker 0x%x ('%s') at %i.\n", marker, */
/*                  jpeg_marker_get_name (marker), o + i); */
//#endif

		/* Append this section */
		jpeg_data_append_section (data);
/*         printf("jpeg_data_load_data() data->count:%ld\n", data->count ); */
		s = &data->sections[data->count - 1];
		s->marker = marker;
		o += i + 1;

        s->content.generic.size = 0;
        s->content.generic.data = 0;
/*         printf("jpeg_data_load_data() s->marker\n"); */
...
/*                 printf("jpeg_data_load_data() default... len:%ld data:%lx\n", */
/*                        len, */
/*                        s->content.generic.data ); */
...
/*     printf("jpeg_data_load_data(end) data->count:%ld\n", data->count ); */
}
...
/*     printf("jpeg_data_set_exif_data() data->count:%ld\n", data->count ); */
    
	section = jpeg_data_get_section (data, JPEG_MARKER_APP1);
	if (!section) {
/*         printf("jpeg_data_set_exif_data( !sec ) data->count:%ld\n", data->count ); */
		jpeg_data_append_section (data);

        
/* 		section = &data->sections[data->count - 1]; */
        
/* 		memmove (&data->sections[2], &data->sections[1], data->count - 1); */
/* 		section = &data->sections[1]; */
/*         section->content.generic.size = 0; */
/*         section->content.generic.data = 0; */

/* 		section = &data->sections[data->count - 1]; */
/*         section->marker = JPEG_MARKER_APP1; */
/*         section->content.app1 = 0; */

        int i=0;
/*         for( i=0; i<data->count; ++i ) */
/*         { */
/*             JPEGSection *psection = &data->sections[ i ]; */
/*             fprintf( stderr, "after append, Found marker 0x%x ('%s') at %i.\n", */
/*                     psection->marker, */
/*                      jpeg_marker_get_name (psection->marker), */
/*                     i ); */
/*         } */
/*         fprintf( stderr, "after printout\n"); */
        
        if( data->count >= 2 )
        {
            for( i=data->count - 2; i > 0; --i )
            {
                if( data->sections[ i ].marker == JPEG_MARKER_SOI
                    || data->sections[ i ].marker == JPEG_MARKER_APP0
                    || data->sections[ i ].marker == JPEG_MARKER_APP1
                    )
                {
                    break;
                }
                
                JPEGSection *cur  = &data->sections[ i+1 ];
                JPEGSection *prev = &data->sections[ i   ];

/*                 fprintf( stderr, "swapping %s with previous marker %s\n", */
/*                          jpeg_marker_get_name (cur->marker), */
/*                          jpeg_marker_get_name (prev->marker) ); */

                JPEGSection t = data->sections[ i ];
...
/* 		section = &data->sections[1]; */
	...
	section->marker = JPEG_MARKER_APP1;
	section->content.generic.size = exif_data->size;
