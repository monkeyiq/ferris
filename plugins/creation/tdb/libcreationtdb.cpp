...
*******************************************************************************
    ...
    $Id: libcreationtdb.cpp,v 1.2 2010/09/24 21:31:53 ben Exp $
...
namespace Ferris
{
    ...
    class CreationStatelessFunctorTDB
        :
    ...
    extern "C"
    {
        ...
        Create()
        {
            return new CreationStatelessFunctorTDB();
        }
    };
    ...
    fh_context
    CreationStatelessFunctorTDB::create( fh_context c, fh_context md )
    {
#ifdef FERRIS_HAVE_TDB
    
        fh_context newc = SubCreate_file( c, md );
        LG_FERRISCREATE_D << "Create tdb at newc->getDirPath:" << newc->getURL() << endl;
        ...
        TDB_CONTEXT* db = tdb_open( (char*)newc->getDirPath().c_str(), 0,
        ...
        {
            fh_stringstream ss;
            ss << "SL_SubCreate_dir() error creating tdb file"
               << " URL:" << newc->getURL()
               << endl;
