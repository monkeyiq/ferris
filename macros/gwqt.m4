...
if test x"$QTDIR" = x ; then
    AC_MSG_RESULT([*** QTDIR must be defined, or --with-qtdir option given])
    return;
fi
AC_MSG_RESULT([$QTDIR])
...
if test x$MOC = x ; then
        AC_MSG_RESULT([*** moc must be in path])
	return;
fi
...
if test x$UIC = x ; then
        AC_MSG_RESULT([*** uic must be in path])
	return;
fi
    ...
    *)
        ...
        if test x$QT_IS_STATIC = xno ; then
            ...
            if test "x$QT_IS_DYNAMIC" = x;  then
                AC_MSG_RESULT([*** Couldn't find any Qt libraries])
		return;
            fi
        fi
